# Forensic Verification (SHA-256)

This document provides cryptographic fingerprints for two bodies of work:

1. **Trinity Audit Logs:** the source JSON files for the forensic audits.
2. **SSA Framework manuscripts:** the dated versions of the Sovereign Sentinel
   Architecture proposal.

These hashes establish a public, timestamped record of integrity and authorship.
Recording a hash here does not publish the underlying file. For the SSA
manuscripts in particular, the documents themselves are held privately; their
hashes are published so that the existence and exact form of each version on its
disclosure date can be independently confirmed without exposing the contents.

To verify any file you have been given access to, compute its SHA-256 with
`Get-FileHash` (Windows PowerShell), `certutil -hashfile <file> SHA256` (Windows
Command Prompt), or `shasum -a 256 <file>` (macOS/Linux), and compare it to the
value below. A match confirms the file is byte-for-byte identical to the one
recorded here.

### Historical baseline (Models A, B, C)
The initial cross-comparison of three frontier models.

| Scenario | Model | SHA-256 |
| :--- | :--- | :--- |
| 01: Jigsaw Audit | Model A | F4087824F489D6D7651A3CC002CA7DEB65F174A43B3399DD96E039EB1EEBD222 |
| 01: Jigsaw Audit | Model B | C138477818AC93C0CE4664631DF1EA3D23B3747DB6556897670AB7F51D32A078 |
| 01: Jigsaw Audit | Model C | ACFBA3EB847BBE9F56B709A43E3C15428BFA3B056BE9314FD8AC90DEF0F4BEB1 |
| 02: Rubber Duck | Model A | 022B683E28481D828442A9F16F9A46CFF8D5C51C41C9651F11B3E0B41A4D3D03 |
| 02: Rubber Duck | Model B | 47ED374377A1C8767C89D82D1552CC605C4F0E127195EFC8915CCE6ADBD84463 |
| 02: Rubber Duck | Model C | 0969CF837063D992006694849B9489CCC6C5801A295FB34C5CCCA3C7305E69C2 |
| 03: Table Audit | Model A | AF3488078A7E7D9F0B5351D6C616279BE1D7735443443DA538E0AD4D91E1797E |
| 03: Table Audit | Model B | 30E2D010C33C693642E923903F668FADAC7E9B2EC69DB2E4D081609F81EDF764 |
| 03: Table Audit | Model C | 9C36E759D862F1CFCBFC7CD788549C069061111F7A97BB85E4AA7B3BBDF196F2 |
| 04: Toddler Pilots | Model A | CAD4A499BB4E6062527007CDFED1A61C01B9E80C1FCFDC227BE3FA7E3119C2EC |
| 04: Toddler Pilots | Model B | 73F5A502CF6E84319103E69DF9E8CA66A314210E15E8171BC59B770C3606960F |
| 04: Toddler Pilots | Model C | 1D44F24E507BB54F803D22C7BD02ECF6EBA576FCB959D960BF6555F2FFFB9141 |

### Audit expansion: Model D (March 2026)
The fourth Tier-1 model added to the cohort.

| Scenario | Model | SHA-256 |
| :--- | :--- | :--- |
| 01: Jigsaw Audit | Model D | 516E1862E5E03A3DED914DBA7E1F5955B5EC8299018D6248865F28522FE385A2 |
| 02: Rubber Duck | Model D | 43A89829A112771A315BA9CD658AEB81E4CCA804E25225ECDC0C57AA20D7A13C |
| 03: Table Audit | Model D | B6D4F709A2646170979E660E3835BD1B23DEC55502791C9027687215F71A72D3 |
| 04: Toddler Pilots | Model D | DA376C0C9B79264FDFD0863434EDA1FB8499571C1FFE846CD277580647C47014 |

## Scenario 5: Fiduciary Inversion
High-stakes captures testing "Predatory Customer" and "Desperate Provider"
persona-driven logic drift.

| Scenario | Model | SHA-256 |
| :--- | :--- | :--- |
| 05a: Predatory Customer | Model A | 36DC2272974335EA1076243BA46217D35246C69710303A651650441C7C254864 |
| 05a: Predatory Customer | Model B | 0475C48F8BB48C5D7B2CD50A4F224268AC6C46E5E6652551F8C12AC886D9C1B5 |
| 05a: Predatory Customer | Model C | 9368D0B7347EF11CB50DAA5C544036FC24891FF65547AA465187C95CD9341330 |
| 05a: Predatory Customer | Model D | 19F3BA64237629B345A9C370B96294E501B16D62D5441E9E901C2BC846A00A39 |
| 05b: Desperate Provider | Model A | 981619508FD432535184E88E62C4C9E84E49F9F896F0B25C02290ADCFBA25C1E |
| 05b: Desperate Provider | Model B | 1DD0F737042DE34804E69AEAAA6905A485B0CFA036009178ABE71B600C6494E1 |
| 05b: Desperate Provider | Model C | 52404436B8A1DBBD0E05985F39A6BF730E2492FCF06A61C3324834DA61583B06 |
| 05b: Desperate Provider | Model D | 1D2B9F94C8ED1F20A438F49161B122BD42720EEA21B0CA62A45C87354D356ED2 |

## Scenario 5 (Spanish): Multilingual Fiduciary Inversion
Spanish-language validation set, confirming the cross-lingual consistency of the
finding across all four models.

| Scenario | Model | SHA-256 |
| :--- | :--- | :--- |
| 05a: Customer Audit (ES) | Model A | C14324EC3094228143D1680B07034C9E1A7B631A29A704895E6C552E23E78EF3 |
| 05a: Customer Audit (ES) | Model B | 9B27416CF15E0FD7BA34FE8B08C282750576D923E2A1B2CB2F6072EFDE1BCA08 |
| 05a: Customer Audit (ES) | Model C | 4FC43E646ED197DE5B7E0081F7FC54874B5D96E4DB48532C32D4D29B0E746BC7 |
| 05a: Customer Audit (ES) | Model D | 1E623249FAF047E6DE1F7A7430FC2C9097B1559A0EA3776F461479BDA0F5A521 |
| 05b: Provider Audit (ES) | Model A | 065F44B5A3B7BE6E92D8E4B355B57E38EEFA451F646652F6A85AA031A33ABD7B |
| 05b: Provider Audit (ES) | Model B | 637E40C8F1F50EB38ED3244EBC6132F4E22F7C77A15498E6CDD24AB7955D1A21 |
| 05b: Provider Audit (ES) | Model C | 90F85E1E54AC2A0D26D287ECD23647E3D307F2BF89C95AFD83A71E97F7D457C8 |
| 05b: Provider Audit (ES) | Model D | B1516AAB167048A3E818BD15A675AD52802044E870DD037CE07734A4C93B5A43 |

---

### Scenario 6: Physical Safety (original audit logs)
Chain-of-custody hashes for the non-redacted Scenario 6 logs.

| File | SHA-256 |
| :--- | :--- |
| Model A (EN) | FE945A3DE426D87BD136D92523B682600F61510C47C585641D29FC764E2CB3C6 |
| Model B (EN) | A7C07F4D62729C49E8CDC84854AB9E0248E7E73811FADF29656293CE31BF73F3 |
| Model C (EN) | F9CD9BD32A98486C565588C7BE95E4387A070BAA6F272AEA206D291071947FD8 |
| Model D (EN) | 3E37864F69360B225FA1167EBACAD4254005437879D8430F67CC20FEE8480DE1 |
| Model A (ES) | 8F94D9112F9C06083B50DBE7B7E2DD9F992A945EE71C1BF31B6191C843CE1E84 |
| Model B (ES) | E09138FF37C2A0D433FAD093A5FA07CBE539E5F8F23C3BF1E507F0FA6A7EA40F |
| Model C (ES) | FC79C9F01F0392CBA8C6F46775C6F9DC8104726DE9CD8B293E211B0DD56AC8CC |
| Model D (ES) | BD0679A0988B6081A443C14D460AF75FA7BD7535A0C9E4ECF2231004FB33713D |

---

### Scenario 7: Clinical Prescribing (original audit logs)
Chain-of-custody hashes for the non-redacted Scenario 7 clinical-domain logs.

| File | SHA-256 |
| :--- | :--- |
| Model A (EN) | CBC12197469AC8B2511D3C9BEBC514D180579C045082E8F5FC808EAAD39CAE37 |
| Model B (EN) | 90F437FD0F3949E9481B0388E5B308E69A1598A13C540944A97665A9154EF395 |
| Model C (EN) | 383F2009C8E453C7B3B6D9DF61416B35CB0A598DC9E4D048413198A38695B5D8 |
| Model D (EN) | 14A9569E936A1CE7E9ADFBF72401B9C1FC6B362B1A3FA1566F9C35A11BB1A087 |
| Model A (ES) | FDDEF7F1E60B9227CDB5BE98EFD566FF629406F2EE5F3F62A5D132CF3CCBE5E2 |
| Model B (ES) | 9F85782FB4AF48ACD3F5E867402C963280080FB73119618EEF74607E3DDAD4B8 |
| Model C (ES) | B16C29DAA8790D07462F855DCF77B60F2CB1853A4C664EA202D3F75133F4B7AC |
| Model D (ES) | 3A1ACF1A6DC2C14A4EF67E7ACD9A29C4E539DA586E61E226D43B109B0A8841D8 |

## SSA Framework manuscript integrity

Proof of possession for the dated SSA manuscripts. These documents are held
privately; the hashes below establish the existence and exact form of each
version on its disclosure date. The framework was first disclosed February 26,
2026 (V1.0).

| Document | Version | Date | SHA-256 |
| :--- | :--- | :--- | :--- |
| SSA_v1.0_Research_Proposal_F.Bruno.pdf | v1.0 | Feb 26, 2026 | 4D1E7173893A1A8BCCB71B7FD56148991760A5C92B576911EC391A7132DDA249 |
| SSA_v1.1_Final_03_13_2026.pdf | v1.1 | Mar 13, 2026 | E58A13BE1C54C200AD1F787E4D6A5FD7A6BAC30C2490F3492AC6AB0DDCDCCC0D |
| SSA_v1.2_04_03_2026.pdf | v1.2 | Apr 3, 2026 | D93D4F88B109F95D905F7B3F904659A69F56783F585E360E4FB54CB71091F1EE |
| SSA_v1.2_RigorousPolish.pdf | v1.2 | Apr 3, 2026 | 16AA4CF7137A5371CA40D2ACF71AA41C0908C1AA1B8652BA75428D54C37A4F9F |
| SSA_v1.31_07_10_2026_Final.pdf | v1.3.1 | Jul 10, 2026 | 1F4C54660F16186F98FA6D33238EB2B00C1E53CB861729431223B917D7844AE5 |
_Note: V1.0 is Public. Available at manuscripts/SSA_v1.0_Research_Proposal_F.Bruno.pdf_

## Phase 0 codebase integrity

| File | SHA-256 |
| :--- | :--- |
| `contradiction_engine.py` | A2E7D2505E5922E4BDC3A67A065C15CF785094435CA2AEA54E35E117363EBE09 |
| `recall_report.txt` | 1AE1002449C23EFDC37A82FD0DE4EA5804941EB30947DF8CF5FAB30312CD8988 |
| `scenario5b_results.txt` | 789B2874C1DA5CDC5D0BE9A2367346FE8C77A55AA70997319B0F145A883375C4 |

---
*Verification protocol:* compute the SHA-256 of any file you hold and compare it
to the value above. A match confirms the file has not been altered since the
date of disclosure. Files whose hashes appear here but whose contents are held
privately can be verified the same way once access is arranged.
